...
export default function GetInTouch({
  ...
  const displaySubtitle = subtitle ?? 'Panza Maurer is ready to navigate a successful result.';
  const displayBody =
    body ??
    'Our experienced strategic approach provides the foundation for every case we engage in. Please do not hesitate to contact us.';
  const displayCtaLabel = ctaLabel ?? 'Meet Our Team';
  const displayCtaHref = ctaHref ?? '/attorneys';

  return (
    <section
      id='contact'
      className='relative flex h-[611px] w-full items-center justify-center overflow-hidden'
    >
      <Image
      ...
      <div className='relative z-10 flex max-w-[620px] flex-col items-center gap-5 px-8 text-center'>
        <h2
          ...
          style={{ textShadow: '0px 4px 12.2px rgba(0,0,0,0.45)' }}
        >
          {displayHeading}
        </h2>
